...
public class Analisis extends QWidget {
	...
	int j = 0;
        for(Grupo g : tabla.getGramatica().getConjuntos()) {
	    if(String.valueOf(i).equals(g.getNombre())) {
                return j;
            }
            j++;
	}
    ...
    public void paso() {
        ...
        try {
            regla = tabla.get(String.valueOf(getIndex(e)), a);
            if (regla == null) {
                analizar = false;
                return;
            }
        ...
        if ("s".equals(accion)) {
            ...
            id = Integer.parseInt(tabla.get(String.valueOf(getIndex(pila.lastElement())), simbolos.lastElement()));
            pila.push(id);
        }
    ...
    public void setTabla(TablaLALR tabla) {
        this.tabla = tabla;
    }
}
